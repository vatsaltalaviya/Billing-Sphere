    ...
    <div className='h-full w-full xl:px-5'>
      ...
      <div className='w-full py-3 '>
        <form className='flex'>
          ...
          {(mode == "item" || mode == "itemGroup" || mode === "hsn" || mode == "tax"||mode === "brand" ||mode === "unit"||mode === "store")&&<input type="text" className='bg-white border font-medium px-3 py-1 w-full'  value={query} onChange={(e) => dispatch(setItemSearchQuery(e.target.value))} />}
      ...
      <div className={`w-full flex justify-center max-h-[85vh] min-h-[80vh] table-data overflow-y-auto`}>
        ...
        {(mode == "itemGroup" || mode === "hsn" ||mode == "tax"||mode === "brand"||mode === "unit"||mode === "store")&&tableData && (tableData.length > 0 ? <Datatable mode={mode} getitemUrl={getitemUrl} list={tableData}/>:<NoTableData mode={mode}/>)}
