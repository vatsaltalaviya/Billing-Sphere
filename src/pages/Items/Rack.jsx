...
const Rack = () => {
  ...
  const navigate = useNavigate();
    ...
    try {
      ...
      const res = await axios.post(`${import.meta.env.VITE_BASE_URL}/store/createStoreLocation`,storageData,{
        headers:{
          Authorization:`${token}`
        }
      })
  ...
  return (
    <div className="w-full h-full">
      <div className="w-full bg-fuchsia-600 flex items-center justify-center">
        <h1 className="font-semibold text-xl text-white">NEW Store locations</h1>
      </div>
      <div className="md:w-xl border-2 mx-auto mt-30 p-2">
        <form onSubmit={handleSubmit} className="space-y-3">
          <div className="flex flex-col md:flex-row xl:items-center">
            <span className="w-44 font-medium">Item Location</span>
            <input type="text" className="border py-2 px-2 " value={location} onChange={(e)=>setLocation(e.target.value)}/>
          </div>
          
          <div className="flex flex-col md:flex-row xl:items-center">
            <span className="w-44 font-medium">Is Active</span>
            <select className="border py-2 px-2" value={isActive} onChange={(e)=>setIsActive(e.target.value)}>
