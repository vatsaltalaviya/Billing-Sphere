...
const Ledger = () => {
  ...
  const ledgerSidebarData = [
    { name: "New", onClick: () => {}, navigate: "/dashboard/ledger/new" },
    {
      ...
      onClick: () => {},
      navigate:
        itemUrl != null && searchingLedgers.length > 0
          ? `/dashboard/ledger/edit/${itemUrl}`
          : "/dashboard/ledger",
    },
    {
      name: "Delete",
    ...
    },
    {
      name: "Filter",
      onClick: () => {
        setFilterLedger(true);
      },
    },
    {
      name: "Label Prn",
      onClick: () => {
        setshowLabelPrn(true);
      },
    },
    {
      name: "Envelop Prn",
      onClick: () => {
        setshowEnvelopPrn(true);
      },
    },
  ...
  const handleDelete = async () => {
    itemUrl != null && dispatch(deleteLedger({ editId: itemUrl }))
        .unwrap()
        .then(() => {
          dispatch(fetchAllledgers());
        })
  ...
  return (
    <div className="w-full">
      <BasePage
        heading="Ledger Master"
        getitemUrl={(e) => getUrl(e)}
        mode="ledgers"
        Sidebardata={ledgerSidebarData}
        tableData={tableData}
      />
      ...
      {showEnvelopPrn && (
        <EnvelopPrn onClose={() => setshowEnvelopPrn(false)} />
      )}
      {showOpBal && <OpBalance onClose={() => setshowOpBal(false)} />}
      {ShowDeleteAlert && searchingLedgers.length > 0 && (
        <DeleteAlert
          field="Ledger Entry"
          onYes={async () => {
            await handleDelete();
